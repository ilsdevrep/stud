...
const
  MaxVert = 50; { Максимальное кол-во вершин в графе }
...
begin
  ...
  for k := 1 to MaxLevel do
    begin
      XPos := PosInLevel[k] +45;
      YPos := (k * VertInterval) - 30;
      ...
      for i := 1 to Vertex[n].ChildVertexQuant do
        begin
          DT[Vertex[n].ChildVertexNumbs[i]] := DownTime[n] +
            {Vertex[n].TimeToChildVertex[i]*2 +}
  ...
  for i := VertexQuant downto 2 do
    begin
      for k := 1 to Vertex[i].ParentVertexQuant do
        begin
          if UpTime[Vertex[i].ParentVertexNumbs[k]]  > (UpTime[i] - {Vertex[i].TimeFromParentVertex[k]*2 -} Vertex[i].DecisionTime)
            then
             UpTime[Vertex[i].ParentVertexNumbs[k]] := (UpTime[i] - {Vertex[i].TimeFromParentVertex[k]*2 -} Vertex[i].DecisionTime);
        end;
    end;
end;
